...
faq: true
summary:
  - "GX Coin is issued by protocol rules with a fixed ceiling of 1.25 billion coins, minted only as participants onboard, not by any central authority."
  - "A fixed money supply can still support a growing economy, because the velocity of circulation, not the quantity of money, determines how much activity it supports."
  - "The 150 trillion genesis figure is sized to match existing global liquidity rather than added on top of it, so it redistributes the monetary base without causing inflation."
  - "Eligibility rests on one verified human and one permanent identity, enforced by five layers including biometric uniqueness, and is designed to include stateless and undocumented people."
  - "The author claims no formal credentials in economics and asks to be judged on the arithmetic, engineering, and published specifications rather than on authority."
...
The [population-weighted starting distribution](/blog/the-great-liberation) is a starting choice, not a redistribution from any pre-existing holder. Every monetary system in history has had to begin with some initial distribution. Starting with equal per-capita allocation is the most defensible starting point because it treats every human being as an equal stakeholder in the monetary substrate. It is more defensible than starting with the historical accident of current wealth concentration, which itself was never chosen democratically or justified philosophically.
